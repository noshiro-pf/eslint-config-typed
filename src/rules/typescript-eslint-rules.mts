...
export const typescriptEslintRules: TypeScriptEslintRules = {
  ...
  /**
   * Function members written in method notation become bivariant, reducing type safety
  ...
  /** Prevent forgetting compare function when sorting numbers, as `sort` defaults to string comparison. */
  ...
  /**
   * Used to avoid ambiguity with `+` operator.
   * restrict-plus-operands limits usage to only bigint, number, string types,
   * and prefer-template prohibits using `+` for string concatenation.
   * Fix: Use template literals or join string arrays with `join("")`.
   *
   * - A + b -> `${a}${b}`
  ...
  /**
   * Used to avoid implicit casting to Boolean.
   * Prevents overlooking that numbers `0`, `NaN` and string `""` are treated as false in conditionals.
   */
